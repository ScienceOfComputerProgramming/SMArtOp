 ...
 */
public class MatrixComputationSparseDistributedDynamicSorted extends MatrixComputationSparseDistributed{
	
	/**
	 * @param adapter adapter for the middleware
	 */
	public MatrixComputationSparseDistributedDynamicSorted(AdapterDistribution adapter) {
		super(adapter);
	}

	/**
	 * Creates the number of tasks that results of dividing the number of non-zero elements by the PARALLEL_FACTOR. 
	 * If the division is not exact, it creates another task with the remaining non-zero elements.
	 * In addition, rows are sorted according to their sparseness. 
	 * Therefore the first tasks might have more rows than the other tasks.
	 * 
	 * @param m
	...
	protected List<ITask> createTasks(Matrix m, int PARALLEL_FACTOR) {
		...
		List<Integer> sortedRows=m.getSortedRows();
		...
		for(int startRow=0;startRow<m.rowSize();startRow=endRow){
			...
			while(endRow<m.rowSize() && (nonzeros<nonZerosPerTask || (m.getSparcityRow(endRow)==1.0))){				
				...
				nonzeros+=m.getNonZerosRow(sortedRows.get(endRow));

				endRow++;

			}

			List<Integer> sub=sortedRows.subList(startRow, endRow);
			
			Matrix leftSubMatrix = m.getSubmatrix(sub);
